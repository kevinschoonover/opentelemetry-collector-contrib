...
func NewDetector(p component.ProcessorCreateSettings, dcfg internal.DetectorConfig) (internal.Detector, error) {
	...
	consulProvider, err := newConsulMetadata(cfg)
	if err != nil {
		return nil, fmt.Errorf("failed creating detector: %w", err)
	}

	return &Detector{provider: consulProvider, logger: p.Logger}, nil
}
...
func (d *Detector) Detect(ctx context.Context) (resource pdata.Resource, schemaURL string, err error) {
	...
	nodeID, err := d.provider.NodeID(ctx)
	if err != nil {
		return res, "", fmt.Errorf("failed getting consul nodeID: %w", err)
	}

	datacenter, err := d.provider.Datacenter(ctx)
	if err != nil {
		return res, "", fmt.Errorf("failed getting consul datacenter: %w", err)
	}
	...
	attrs.InsertString(conventions.AttributeCloudRegion, datacenter)
	attrs.InsertString(conventions.AttributeHostID, nodeID)
